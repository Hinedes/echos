"""Check body Euler angles and beam z-components in corridor simulation."""
...
# Run simulation with roll/pitch/beam-z logging
az_d=np.linspace(-30,30,7)
check_steps=[1431, 1440, 1460, 1480, 1500, 1520, 1540, 1550, 1560, 1600, 1630, 1652, 1656, 1699]
log=[]
...
for st in range(1700):
 ...
 if st in check_steps:
    roll,pitch,yaw=qeuler(qc)
    log.append((st,roll,pitch,yaw,p[0],p[1]))
    # Check az=6 specifically
    az=np.radians(az_d[6]);db=np.array([np.cos(az),np.sin(az),0]);dw=Rb@db
    log.append((f"  beam z={dw[2]:.4f} range={rf[6]:.4f} hit={rf[6]>=0}"))
    # Check if any beam has non-zero z
    for i in range(7):
        if abs(Rb@np.array([np.cos(np.radians(az_d[i])),np.sin(np.radians(az_d[i])),0])[2])>0.001:
            log.append((f"    az={i} z={(Rb@np.array([np.cos(np.radians(az_d[i])),np.sin(np.radians(az_d[i])),0]))[2]:.4f}",))
...
print("\n=== Body Euler angles at key steps ===")
for entry in log:
    if isinstance(entry, tuple):
        if len(entry)==6:
            st,r,p,y,px,py=entry
            print(f"step {st}: roll={r:+.1f} pitch={p:+.1f} yaw={y:+.1f} pos=({px:.3f},{py:.3f})")
        else:
            print(entry[0])
