...
at=scene.add_sensor(gs.sensors.Raycaster(pattern=gs.sensors.SphericalPattern(angles=(np.array([0.0]),np.array([0.0]))),entity_idx=body.idx,pos_offset=emit_off,max_range=8,no_hit_value=-1.0))
...
class M:
 ...
 def ur(s,ew,dr,rng,mx,hit,eps=0.004):
  ex,ey=ew[0],ew[1]
...
m=M((-2,7,-2,5),0.08);eps=0.004;az_d=np.linspace(-30,30,7);az_c=np.cos(np.radians(az_d));traj=[]
for st in range(350):
 ...
 if st%5==0:dt=at.read();rt=dt.distances.flatten()[0].item()
 ...
 for i in range(7):
  ...
  m.ur(ew,dw,rr,5,h,eps)
 if st%5==0:dwt=Rb@np.array([1,0,0]);ht=rt>=0;m.ur(ew,dwt,rt if ht else 0,8,ht,eps)
 if st<300:tgt=np.array([p[0]+2,0,1]);Tt,qd=ppd(tgt,p,v);arf(np.clip(mxr(Tt,*apd(qd,qc,om)),0,None))
 scene.step();p=body.get_pos().cpu().numpy();traj.append(p.copy())

occ=m.gm();h,w=occ.shape
print(f"Drone: pos=({p[0]:.3f},{p[1]:.3f})")
...
for iy in range(1,h-1):
 for ix in range(1,w-1):
  if occ[iy,ix]==0.5:
   if occ[iy-1,ix]==0 or occ[iy+1,ix]==0 or occ[iy,ix-1]==0 or occ[iy,ix+1]==0:
    fc.append((ix,iy))
print(f"Raw free-adj-unknown: {len(fc)}")

# Focus on the inner corner region
print(f"\nINNER CORNER (x 2.8-3.2, y 1.3-1.7):")
for iy in range(h):
 for ix in range(w):
  wx,wy=m.g2w(ix,iy)
  if 2.8<=wx<=3.2 and 1.3<=wy<=1.7:
   v=occ[iy,ix];iv=inf[iy,ix];is_fc=(ix,iy)in fc
   os="U"if v==0 else("F"if v==0.5 else"O")
   print(f"  ({wx:.2f},{wy:.2f}) occ={os} inf={int(iv)} fc={'*'if is_fc else'.'} views={m.views[iy,ix]} hits={m.hits[iy,ix]}")

# Vertical corridor entrance
print(f"\nVERTICAL ENTRANCE (x 2.8-3.5, y 1.4-2.0):")
for iy in range(h):
 for ix in range(w):
  wx,wy=m.g2w(ix,iy)
  if 2.8<=wx<=3.5 and 1.4<=wy<=2.0:
   v=occ[iy,ix];iv=inf[iy,ix];is_fc=(ix,iy)in fc
   os="U"if v==0 else("F"if v==0.5 else"O")
   print(f"  ({wx:.2f},{wy:.2f}) occ={os} inf={int(iv)} fc={'*'if is_fc else'.'} views={m.views[iy,ix]} hits={m.hits[iy,ix]}")
...
# Frontier x-range
if fc:
 xs=[m.g2w(c[0],c[1])[0]for c in fc]
 print(f"\nFrontier x-range: {min(xs):.2f} to {max(xs):.2f}")
 print(f"Drone x: {p[0]:.2f}")
 print(f"Forward filter passes cx >= {p[0]-0.5:.2f}")
 near=[c for c in fc if m.g2w(c[0],c[1])[0]>=p[0]-0.5]
 print(f"After forward filter: {len(near)} / {len(fc)}")

# Cells behind the wall
print(f"\nBEHIND WALL (x 3.0-3.5, y 1.6-1.8):")
for iy in range(h):
 for ix in range(w):
  wx,wy=m.g2w(ix,iy)
  if 3.0<=wx<=3.5 and 1.6<=wy<=1.8:
   v=occ[iy,ix];iv=inf[iy,ix];is_fc=(ix,iy)in fc
   os="U"if v==0 else("F"if v==0.5 else"O")
   print(f"  ({wx:.2f},{wy:.2f}) occ={os} inf={int(iv)} fc={'*'if is_fc else'.'} views={m.views[iy,ix]} hits={m.hits[iy,ix]}")

with open("/workspace/diag_map.pgm","w")as f:
 f.write(f"P2\n{w} {h}\n255\n")
 for iy in range(h-1,-1,-1):
  ...
  f.write(" ".join(row)+"\n")
print(f"\nMap: /workspace/diag_map.pgm ({w}x{h})")
